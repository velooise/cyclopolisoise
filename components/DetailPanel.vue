  ...
  >
    <template #title>
      <div class="flex gap-2 align-middle items-center">
        <h2 class="text-lg font-medium leading-6">Voie {{ selectedLine }}</h2>
        <a
          v-if="pathToLine"
          :to="pathToLine"
...
const pathToLine = computed(() => (selectedLine.value ? `/velo-oise-${selectedLine.value}` : null));
